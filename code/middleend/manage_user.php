...
while ($row = $result->fetch_assoc()) {
    ...
    // Determine actual role for the edit/delete links
    $actualRole = $role;
    if ($role === 'all' && isset($row['role'])) {
        $actualRole = strtolower($row['role']);
    }

    if ($context === 'full') {
        $actions .= "<a href='../middleend/edit_user.php?id={$row['id']}&role={$actualRole}'>Edit</a> | ";
        $actions .= "<a href='../middleend/delete_user.php?id={$row['id']}&role={$actualRole}' onclick='return confirm(\"Are you sure?\")'>Delete</a>";
    } elseif ($context === 'edit') {
        $actions .= "<a href='../middleend/edit_user.php?id={$row['id']}&role={$actualRole}'>Edit</a>";
